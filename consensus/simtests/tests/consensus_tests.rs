...
mod consensus_tests {
    ...
    use consensus_core::{
        BlockAPI, BlockStatus, CommitIndex, CommittedSubDag, TransactionVerifier, ValidationError,
    };
        ...
        let _handle = tokio::spawn(async move {
            ...
            for i in 0..NUM_TRANSACTIONS {
                ...
                if let Some(client) = authority.transaction_client_if_running() {
                    let _ = client.submit(vec![txn]).await;
                }
    ...
    async fn test_consensus_rolling_restarts_all_validators() {
        ...
        let load_handle = tokio::spawn(async move {
            ...
            loop {
                ...
                if let Some(client) = authority.transaction_client_if_running() {
                    let _ = client
                        .submit(vec![transaction_id.to_le_bytes().to_vec()])
                        .await;
                }
                transaction_id += 1;
                sleep(Duration::from_millis(20)).await;
                ...
                join_set.spawn({
                    ...
                    async move {
                        let Ok((_block_ref, _indexes, status_waiter)) =
                            client.submit(transactions).await
                        else {
