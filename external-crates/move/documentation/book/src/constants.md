...
Constants do not take visibility modifiers such as `public` or `public(package)`. By default,
`const` values can be used only in the declaring module. However, as a convenience, they can be
used across modules in [unit tests attributes](./unit-testing.md).

### Cross-Module Usage

With the `2024.alpha` edition, a constant can be used from any module in the *same package* as
its declaring module, both in code and in the definitions of other constants:
...
Constants remain internal to their package: using a constant from another package is an error.
...
`2024.alpha` edition, to constants of other modules in the same package, as described in
