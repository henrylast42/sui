...
pub(crate) fn read_built_modules(package_path: &Path) -> Result<Vec<CompiledModule>, Error> {
...
mod tests {
    ...
    fn reads_root_modules_only() {
        ...
        assert_eq!(read_built_modules(pkg).unwrap().len(), 1);
    }
    ...
    fn errors_when_unbuilt() {
        ...
        assert!(read_built_modules(tmp.path()).is_err());
    }
}
