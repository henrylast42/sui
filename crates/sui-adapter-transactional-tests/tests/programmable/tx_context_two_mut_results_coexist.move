...
// Under --allow-references-in-ptbs, two `&mut Y` results from two calls that each take
// `&mut TxContext` can coexist and both be written through. If the injected TxContext
// participated in the borrow graph, both results would mutably extend a shared TxContext
// root and the second `borrow_mut` (and every use after it) would be rejected.
